...
class Psd {
  ...
  async init(file) {
    return new Promise((jsonResolve) => {
      let fileName = file.name.substring(0, file.name.lastIndexOf(".")) + ".png";
      // psd文件
      const url = URL.createObjectURL(file);
      // 解析psd文件
      PSD.fromURL(url)
        .then(async (psd) => {
          const { backgroundImage, width, height } = await this.getPsdBgImage(psd);
          // 获取图层数据
            ...
            .then((a) => {
              if (a) {
                ...
                if (a.type == "text") {
                  ...
                  newTextObj.fontSize = Math.round(fontSize * transY * 100) * 0.01; // 32 ✔
